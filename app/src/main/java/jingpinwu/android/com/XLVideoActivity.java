   ...
   // ProgressDialog progressDialog;
    ...
    Runnable runnablehuanchong = new Runnable()
    {
        ...
        public void run()
        {
            ...
            try
            {
                Document document = Jsoup.connect("http://vip.qtxw.com/")
                        .timeout(5000)
                        .get();
                Element element = document.getElementById("jk");
                if(element!=null)
                {
                    Elements elem = element.select("option");
                    if(elem!=null && elem.size()>JXQZ_url_index)
                    {
                        JXQZ_url = elem.get(JXQZ_url_index).attr("value");
                        if(JXQZ_url.startsWith("//"))
                        {
                            JXQZ_url = "https:"+JXQZ_url;
                        }
                        Log.e("AAAA","JXQZ_url="+JXQZ_url);
                    }
                    else if(elem.size()<=JXQZ_url_index)
                    {

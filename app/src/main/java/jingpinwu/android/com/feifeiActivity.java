...
public class feifeiActivity extends Activity implements EventListener
{
    ...
    private String vtottext = "你好，我叫飞飞";
    ...
    Handler handler = new Handler()
    {
        ...
        public void handleMessage(Message msg)
        {
            ...
            {
                text_voice.setText("小飞来啦，长按话筒和我聊天吧");
            }
    ...
    SpeechSynthesizerListener TtoVlistener = new SpeechSynthesizerListener()
    {
        ...
        {
            Log.e("AAAA","xiaofei onSynthesizeStart");
        }

        @Override
        public void onSynthesizeDataArrived(String s, byte[] bytes, int i, int i1) {
            Log.e("AAAA","xiaofei onSynthesizeDataArrived");
        }

        @Override
        public void onSynthesizeFinish(String s)
        {
            Log.e("AAAA","xiaofei onSynthesizeFinish");
        }

        @Override
        public void onSpeechStart(String s)
        {
            Log.e("AAAA","xiaofei onSpeechStart");
        }

        @Override
        public void onSpeechProgressChanged(String s, int i)
        {
            Log.e("AAAA","xiaofei onSpeechProgressChanged");
        }
        ...
        {
            Log.e("AAAA","xiaofei onError:"+speechError.toString());
        }
    };
        ...
        try
        {
            ...
            if(connection.getResponseCode()==200)
            {
                ...
                if(vtottext.length()>0 && vtottext.contains("{"))
                {
                    ...
                    {
                        vtottext = "小飞有点累了，你也休息一会吧";
                    }
                }
                if(vtottext.contains("[name]"))
                {
                    vtottext = vtottext.replace("[name]","大笨猪");
                }
                if(vtottext.contains("[cqname]"))
                {
                    vtottext = vtottext.replace("[cqname]","小飞");
                }
                if(vtottext.contains("茉莉"))
                {
                    vtottext = vtottext.replace("茉莉","小飞");
                }
            }
            else
            {
